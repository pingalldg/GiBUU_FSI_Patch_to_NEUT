...
def generate_fortran_assignment(j):
    ...
    if np.sum(isatPV_idx)==5:
        ...
        realparticle_external1 =  np.array([0., 0., 0., 0.])
        realparticle_vel1 =  np.array([0., 0., 0.])
        ...
        if ((mode_array[j]==12)& (mode_array[j]==13)):
            lines = [
                ...
                f"{realparticle_external1[0]} {realparticle_external1[1]} {realparticle_external1[2]} {realparticle_external1[3]}",
                f"{realparticle_vel1[0]} {realparticle_vel1[1]} {realparticle_vel1[2]}",                
                ...
                f"{phiLepton}",
                f"{1} {0} {0} {0} {0} {0} {2} {1}",  # static line
                f"{2}"
            ]
            
        elif(mode_array[j]==11):
            lines = [
                f"{realparticle_external[0]} {realparticle_external[1]} {realparticle_external[2]} {realparticle_external[3]}",
                f"{realparticle_vel[0]} {realparticle_vel[1]} {realparticle_vel[2]}",
                f"{realparticle_external1[0]} {realparticle_external1[1]} {realparticle_external1[2]} {realparticle_external1[3]}",
        ...
        lines = [
            ...
            f"{betacm[0]} {betacm[1]} {betacm[2]}",
            f"{phiLepton}",
            f"{1} {0} {0} {0} {1} {0} {1} {1}",  # static line
            f"{1}"
        ]
        return '\n'.join(lines)
    
    elif(np.sum(isatPV_idx)==6):

        realparticle_external = moma(nuc_idx, j)
        realparticle_vel = realparticle_external[1:4] / realparticle_external[0]
        realparticle_external1 = moma(nuc_idx+1, j)
        realparticle_vel1 = realparticle_external1[1:4] / realparticle_external1[0]
        realparticle_external_pdg1=pdg_array[j,nuc_idx+1]

        finalstate_external1 = moma(nuc_idx + 3, j)
        finalstate_external2= moma(nuc_idx + 4,j)
        finalstate_pdg=pdg_array[j,nuc_idx+3]
        finalstate_pdg1=pdg_array[j,nuc_idx+4]
        if finalstate_pdg==2212:
            charge_pdg=1
        elif finalstate_pdg==2112 :
            charge_pdg=0    

        if finalstate_pdg1==2212:
            charge_pdg1=1
        elif finalstate_pdg1==2112 :
            charge_pdg1=0  
        ...
        W = invariant_mass(moma(nu_idx, j), moma(lep_idx, j), moma(nuc_idx, j)+moma(nuc_idx+1, j))
        W_free = W
        W_rec = W

        betacm = beta(moma(nuc_idx, j)+moma(nuc_idx+1, j), boson)
        pcm = -lorentz_boost(betacm, realparticle_external+realparticle_external1)
        ...
        lines = [
            ...
            f"{betacm[0]} {betacm[1]} {betacm[2]}",
            f"{phiLepton}",
            f"{1} {0} {1} {charge_pdg+charge_pdg1-1} {1} {charge_pdg} {1} {charge_pdg1}",  # static line
            f"{35}"
        ]
        return '\n'.join(lines)
